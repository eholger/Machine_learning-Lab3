...
if (0):
    def trainBoost(base_classifier, X, labels, T):
        ...
        #repeat step 1-4 T times
        for t in range(0,T):
            #step 1 train classifier with W
            new_classifier = base_classifier.trainClassifier(X,labels,W)
            #classifies the new classifier 
            h.append(new_classifier.classify(X))
            # calc delta function, 1 if h[i] = labels[i] , 0 otherwise.
            delta = [h[t][i] == labels[i] for i in range(0,len(h))]
            # calc the error 
    ...
    def classifyBayes(X,prior,mu,sigma):
        ...
        for i in range(0,len(X)):
            ...
            for k in range(0,len(mu)):
                inv = (X[i]-mu[k])
                inv = [inv[0]*sigma[k][1][1], inv[0]*sigma[k][0][0]]*inv
                inv = np.sum(inv)*1/float(sigma[k][0][0]*sigma[k][1][1])
                delta = -1/float(2)*np.log(np.sqrt(sigma[k][0][0]**2+sigma[k][1][1]**2))-1/float(2)*inv + np.log(prior[k])
                if delta>deltaPrev:
                    # saves the best classification for the current data point. 
                    bestClass = k
                deltaPrev = delta
            # adds the bestClass to the final vector that contains the classification of all the data points. 
            N.append(bestClass)


        print (N)
        #vector that contains the predictions.
        return N


    def computePrior(labels,W):
        # collect weights classwise and sum
        cl = np.unique(labels)
        prior = []
        for k in cl:
            wTemp = 0
            for i in range(0,len(W)):
                if k == labels[i]:
                    wTemp += W[i]
            prior.append((float(wTemp)))
        print (prior)

        # non modded version of computePrior
        i = len((np.unique(labels)))
        prior = np.zeros((i))
        prior.fill(1/float(i))
        print ('something')
        print (i)
        print (prior)
        return prior
        ...
        for ii in cl:
            ...
            for i in range(0,len(X)):
                if ii == labels[i]:
                    ...
                    temp.append(X[i])
                    wTemp.append(W[i])
            # saves the mean for each classification 
            print ('summaX')
            print (summaX)
            print (count)
            newPoint = [summaX/float(count), summaY/float(count)]
            mu.append(newPoint)
            # sorterad efter classification
            xSort.append(temp)
            # Weights are sorted 
            wSort.append(wTemp)

        print ('count')
        #x value in the first class and first coordinate
        print ((xSort[1][0]-mu[1])**2)
        # calc the variance of each class, that is the diagonal in the covariance matrix 
        d0 = []
        d1 = []
        sigma = []
        part = int(len(X)/len(cl))
        for ii in cl:
            count = 0 
            element = 0
            for i in range(0,part):
                element += wSort[ii][i]*(xSort[ii][i] - mu[ii])**2
                count += wSort[ii][i]
            # saves the variance 
            d0.append(element[0]/float(count))
            d1.append(element[1]/float(count))
            #print ()
            sigma.append(np.diag((element[0]/float(count),element[1]/float(count))))
        print (count)
        #sigma = np.diag(d)
        print ('d')
        print (d1)
...
def plotBoundary(classifier, dataset='iris', split=0.7):
    ...
    #plt.savefig('BayesClassifier.png')
    plt.show()
